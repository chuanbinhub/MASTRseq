...
PART2_run_str_counter.py
Written by Han-Seul Ryu (Last modified Sept 16th 2024)
'''


'''
INPUT: (1) aligned, demultiplexed, and sorted bam file from PART1c, (2) desired output directory (output directory will be created for you if it doesn't exist)

OUTPUT: (1) filters.txt file that stores your filtering parameters, (2) counts.txt files with read names and STR count

IMPORTANT: Please modify input section in main() function if you would like to modify the filtering parameters.
'''
...
def main():
    ...
    example command
    ----------------
    python PART2_run_str_counter.py [input_bam_file] [desired_output_directory]

    python PART2_run_str_counter.py ./example_c9orf72_ALS_demultiplexed_barcode19.bam ./example_c9orf72_ALS_demultiplexed_barcode19_filter1
    ...
    #########################################################################################################################
    ################################# This is where you can modify the filtering parameters #################################

    #parameters below were used by MASTR-seq authors for C9orf72 G2C4 counting. Please see subfolders [HTT_CAG] and [FMR1_CGG] for parameters used for CAG and CGG counting.
    ...
    base_file_name = input_bam.split('/')[-1].rstrip('.bam')
    ...
    for x in region:
        ...
        if seq.count("TGTGTGTGTGTGTGTGTGTG") >= 4:
            no_pass += 1
            continue
        if seq.count("TAATAATAATAATAATAATAATAATAATAA") >= 1:
            no_pass += 1
            continue
        if seq.count("CAACAACAACAACAACAACAACAACAACAA") >= 2:
            no_pass += 1
            continue       
